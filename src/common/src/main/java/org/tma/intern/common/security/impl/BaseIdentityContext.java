...
@Slf4j
public class BaseIdentityContext implements IdentityContext {
    ...
    private Region extractCountryCode(List<String> groupPaths) {
        for (String path : groupPaths) {
            if (path.startsWith("/global/")) {
                ...
                if (parts.length > 2) {
                    return Region.valueOf(parts[2].toUpperCase()); // phần tử sau "/global/"
                }
            }
        }
        throw new RuntimeException("Cannot extract region !!!");
    }
