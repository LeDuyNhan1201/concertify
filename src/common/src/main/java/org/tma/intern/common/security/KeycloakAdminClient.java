...
import org.tma.intern.common.dto.IdentityRole;
import org.tma.intern.common.dto.Region;
import org.tma.intern.common.dto.ResourceType;
...
//    }
    ...
    public Uni<String> createUser(IdentityUser user, IdentityGroup group, Region region) {
        ...
        newUser.setGroups(Stream.of(group).map(identityGroup ->
            MessageFormat.format("{0}/{1}/{2}", GROUP_PREFIX, region.code, identityGroup.type)).toList());

        return isGroupExists(group, region)
            .onItem().transformToUni(exists -> {
                if (!exists) {
                    return Uni.createFrom().failure(new RuntimeException(
                        "Group does not exist: " + group.type + " in region: " + region.code));
                }
    ...
    public Uni<Boolean> isGroupExists(IdentityGroup group, Region region) {
        return Uni.createFrom().item(() ->
            findGroupByPath(MessageFormat.format("{0}/{1}/{2}", GROUP_PREFIX, region.code, group.type)).isPresent())
            .runSubscriptionOn(Infrastructure.getDefaultWorkerPool());
    }
    ...
    public String createSpecificGroup(GroupsResource groups, String parentId, IdentityGroup group, Region region) {
        ...
        String roleName = MessageFormat.format("{0}_{1}", generateRoleBaseOnGroup(group).value, region.code);
        ...
        Optional<GroupRepresentation> created = findGroupByPath(MessageFormat
            .format("{0}/{1}/{2}", GROUP_PREFIX, region.code, group.type));
        ...
        try (Response response = groups.group(parentId).subGroup(newGroup)) {
            ...
            if (status == Response.Status.CREATED.getStatusCode()) {
                ...
                groups.group(groupId).roles().realmLevel().add(List.of(roles.get(roleName).toRepresentation()));
                return groupId;
            } else {
    ...
    private String createGroupBlocking(IdentityGroup group, Region region) {
        ...
        Optional<GroupRepresentation> checkRegionGroup =
            findGroupByPath(MessageFormat.format("{0}/{1}", GROUP_PREFIX, region.code));
    ...
    private String createResourceBlocking(ResourceType resourceType, Region region) {
        ...
        String clientId = generateClientIdBaseOnResourceType(resourceType);

        ResourceRepresentation resource = new ResourceRepresentation();
        resource.setName(MessageFormat.format("{0} {1}", region.name(), resourceType.name));
        resource.setType(MessageFormat.format("urn:{0}:resources:{1}_{2}", clientId, resourceType.type, region.code));
        ...
        try (Response response = keycloak.realm(REALM)
            .clients()
            .get(clients.findByClientId(clientId).get(0).getId())
            .authorization()
            .resources()
            .create(resource)) {
            int status = response.getStatus();
    ...
    private String generateClientIdBaseOnResourceType(ResourceType resourceType) {
        return switch (resourceType) {
            case CONCERT -> "concert-service";
    ...
    public Optional<GroupRepresentation> findGroupByPath(String path) {
        ...
        Optional<GroupRepresentation> currentOpt = topGroups.stream()
            .filter(gr -> gr.getName().equalsIgnoreCase(names.getFirst())).findFirst();
