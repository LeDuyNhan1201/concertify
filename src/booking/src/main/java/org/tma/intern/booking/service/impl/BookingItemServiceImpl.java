...
public class BookingItemServiceImpl extends BaseService implements BookingItemService {
    ...
    public Uni<List<String>> delete(List<String> ids) {
        return Multi.createFrom().iterable(ids).map(ObjectId::new)
            .onItem().transformToUniAndMerge(this::delete)
            .collect().asList();
    }

    @Override
    public Uni<Boolean> isExisted(String seatId) {
        return bookingItemRepository.find("seat_id", seatId).list()
            .onItem().transform(items -> !items.isEmpty());
    }
    ...
    public Uni<List<BookingItemResponse.Detail>> findAllByBookingId(String bookingId) {
        return bookingItemRepository.find("booking_id", bookingId).list()
            .onItem().transform(items -> items.stream().map(bookingItemMapper::toDto).toList());
    }
    ...
    public Multi<BookingItem> findByIds(List<String> ids) {
        ...
        return bookingItemRepository.find("_id in ?1", objectIds).stream();
    }
